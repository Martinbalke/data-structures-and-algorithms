...
const matchMonth = (input) => {
  let match = false;
  if(/^oct$/.test(input)) match = true;
  if (/^october$/.test(input)) match = true;
  if (/^Oct$/.test(input)) match = true;
  if (/^October$/.test(input)) match = true;
  return match;
};
...
const noPunctuation = str => {
  let regx = /[a-z0-9]$/i
  let newStrArray = str.split(' ').filter((word) => {
...
const findShells = (str) => {
  let regx = /ells$/i
  return str.split(' ').map((word) => word.replace('.', '')).filter((word) => regx.test(word))
};
