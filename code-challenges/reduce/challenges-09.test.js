...
const countNumberOfChildren = (arr) => {
  return arr.reduce( (acc, character) => {
    if(character.children) acc += character.children.length;
    return acc;
  }, 0)
};
...
const extractStat = (statName, arr) => {
  return arr.reduce( (acc, stats) => {
    if(stats.stat.name === statName) acc = stats;
    return acc;
  }, null)
};
...
const extractChildren = (arr) => {
  return arr
    .filter((character) => character.name.match(/a/i))
