...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  for (let i = 0; i < input.length; i++) {
    let thing = input[i]
      .filter((num) => {if (typeof (num) === 'number' && num % 5 === 0) return true;})
      .map((num) => Math.pow(2, num));
    poweredArray.push(thing)
  }
  return poweredArray;
};
...
let findMaleAndFemale = (data) => {
  ...
  data
    .filter((character) => character.gender === 'female' || character.gender === 'male')
    .forEach((character) => names.push(character.name))
...
let findShortest = (data) => {
  ...
  data.reduce((acc, character) => {
    if (acc === 0 || acc > parseInt(character.height)) {
      acc = parseInt(character.height)
