...
const listFoods = (recipe) => {
  const sliceFunction = (string, word) => {
  ...
  recipe.ingredients.forEach((ingredient) => {
    ...
    ingredient = sliceFunction(ingredient, 'pounds');
    ingredient = sliceFunction(ingredient, 'pound');
    ingredient = sliceFunction(ingredient, 'cups');
    ingredient = sliceFunction(ingredient, 'gallons');
    ingredient = sliceFunction(ingredient, 'sized');
...
const splitFoods = (recipe) => {
  let result = [];
  recipe.ingredients.forEach((ingredient) => {
    ingredient = ingredient.split(' ').splice(2, 3).join(' ');
...
const stepActions = (recipe) => {
  ...
  recipe.steps.forEach( step => {
    result.push(step.split(' ').splice(0,1));
  });
  return result.flat();
};
...
const removeVowels = (str) => {
  str = str.split('');
  let noVowels = str.filter((letter) => {
...
const extractVowels = (str) => {
  ...
  str.forEach((letter) => {
    if (letter === 'a' || letter === 'e' || letter === 'i' || letter === 'o' || letter === 'u') {
      extracted[1].push(letter);
    }
    else {
      extracted[0].push(letter);
    }
  })
  extracted[1] = extracted[1].sort().join('');
  extracted[0] = extracted[0].join('');
  return extracted;
};
